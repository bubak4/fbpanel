// Time-stamp: < power_supply.c (2015-02-15 15:42) >
// Author: Bubak <martinslouf@users.sf.net>
...
ac_parse(ac* ac)
{
    ...
    if (ac->path != NULL) {
        ...
        if (hash != NULL) {
            ...
            // ac online
            tmp_value = (gchar*) g_hash_table_lookup(hash,
                SYS_ACPI_UEVENT_AC_ONLINE_KEY);
            if (tmp_value != NULL) {
                ac->online = strcmp(SYS_ACPI_UEVENT_AC_ONLINE_VALUE,
                    tmp_value) == 0;
            }
...
bat_parse(bat* bat)
{
    ...
    if (bat->path != NULL) {
        ...
        if (hash != NULL) {
            ...
            // battery status
            tmp_value = (gchar*) g_hash_table_lookup(hash,
                SYS_ACPI_UEVENT_BAT_STATUS_KEY);
            if (tmp_value != NULL) {
            ...
            tmp_value = (gchar*) g_hash_table_lookup(hash,
                SYS_ACPI_UEVENT_BAT_CAPACITY_KEY);
            if (tmp_value != NULL) {
                ...
                if (tmp_value != NULL) { // ac off
                    ...
                    tmp_value = (gchar*) g_hash_table_lookup(hash,
                        SYS_ACPI_UEVENT_BAT_CHARGE_NOW_KEY);
                    if (tmp_value != NULL) { // ac on
                        tmp = g_ascii_strtod(tmp_value, NULL);
                        tmp_value = (gchar*) g_hash_table_lookup(hash,
                            SYS_ACPI_UEVENT_BAT_CHARGE_FULL_KEY);
                        if (tmp_value != NULL && tmp > 0.0d) {
                            tmp = tmp / g_ascii_strtod(tmp_value, NULL)
                                * 100.0d;
                            bat->capacity = tmp;
                        }
                    }
            ...
            while ((tmp = g_dir_read_name(dir)) != NULL) {
                ...
                if (g_file_test(filename->str, G_FILE_TEST_IS_REGULAR)) {
                    ...
                    if (strcmp(SYS_ACPI_TYPE_AC, contents) == 0) {
                        ...
                        g_fprintf(stderr, "unsupported power supply type %s",
                            contents);
                    }
                    g_free(contents);
                }
...
power_supply_get_bat_capacity(power_supply* ps)
{
    ...
    return (gfloat) (total_bat_capacity / bat_count);
}
